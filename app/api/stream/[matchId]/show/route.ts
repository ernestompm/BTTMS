...
/**
 * POST /api/stream/:matchId/show
 * Body: { "key": "<graphic_key>", "data"?: <any> }
 * Mostrar gráfico en PROGRAMA. Si ya está visible, solo actualiza data.
 */
export async function POST(req: NextRequest, { params }: { params: Promise<{ matchId: string }> }) {
  ...
  await svc.from('stream_events').insert({ session_id: session.id, kind: 'api_show', graphic: body.key, payload: { data: body.data ?? null } })
